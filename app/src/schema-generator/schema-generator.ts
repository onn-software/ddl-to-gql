...
export class SchemaGenerator {
  ...
  private generateGqlSchemaQueries(tableDefs: TableDef[], gqlNoRoot:boolean) {
    ...
    tableDefs.forEach(table => {
      queries.push(`  ${table.tableName}(paginate: Paginate, orderBy: OrderBy, where: [WhereClause!], distinct: [String!]): Paginated${Globals.getGqlName(table.tableName)}!`);
    })
  ...
  private generateGqlSchemaMutations(tableDefs: TableDef[], gqlNoRoot: boolean) {
    ...
    tableDefs.forEach(table => {
      mutations.push(`  action_${table.tableName}(action: MutationAction!, where: [WhereClause!]!, value: ${Globals.getGqlName(table.tableName)}_upsert!): MutationResult!`);
